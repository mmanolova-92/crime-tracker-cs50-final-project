...
import os, sys, json, geocoder, datetime
import pandas as pd
...
def register():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO users (userName, password, firstName, lastName, streetName, city, PLZ) VALUES (?, ?, ?, ?, ?, ?, ?)", username, hash_password, firstname, lastname, streetname, city, plz)
...
def index():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO crimeReports_news (date, time, crimeType, location, latLong, title, username, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", crimeDate_input.replace('.','/'), crimeTime_input, crimeType_input, crimeLocation_input, crimeLatLong_input, crimeDescription_input, username, "user")
        return redirect("/")
        ...
        with open('crimes.json') as json_file:
            ...
            for entry in data:
                ...
                elif entry['location'] == 'bundesweit' or entry['location'] == 'berlinweit' or entry['location'] == 'bezirksübergreifend':
                ...
                title = entry['title']
                link = 'https://www.berlin.de' + entry['link'][0]
                crimeType = ', '.join(entry['crimeType'])
                source = "Polizeipräsident"

                db.execute("INSERT INTO crimeReports_news (date, time, location, latLong, title, crimeType, source, link) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", date.replace('.','/'), time, location, latLong, title, crimeType, source, link)
        ...
        reports_news = db.execute("SELECT title, location, latLong, link, source, date, time, crimeType FROM crimeReports_news ORDER BY date DESC, time DESC")
...
def signalsLog():
    ...
    username = session["user_id"]
    signals_list = db.execute("SELECT date, time, location, crimeType, title FROM crimeReports_news WHERE username = ?", username)
    signals_df = pd.DataFrame(signals_list)

    signals_df = signals_df[['date','time','location','crimeType','title']]
    signals_df.columns = ['Datum', 'Zeit', 'Ort', 'Straftatart', 'Titel']
