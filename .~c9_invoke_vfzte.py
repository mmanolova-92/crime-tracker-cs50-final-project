...
#def run_scrapy():
        ...
        #os.remove("crimes.json")

    #yield runner.crawl(crimeSpider)
    #reactor.stop()
    ...
    if request.method == "POST":
        ...
        with open('crimes.json') as json_file:
            ...
            for entry in data:
                ...
                db.execute("INSERT INTO crimeReports_news (date, time, location, latLong, link, title, crimeType, source) VALUES (?, ?, ?, ?, ?, ?, ?)", date.replace('.','/'), time, location, latLong, link, title, crimeType, source)
