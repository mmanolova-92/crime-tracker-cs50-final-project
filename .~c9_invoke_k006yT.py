import os, sys, json, geocoder, sqlite3
import pandas as pd
...
def register():
    ...
    if request.method == "GET":
        ...
        existing_users_raw = cur.execute("SELECT distinct username FROM users")
        ...
        cur.execute("INSERT INTO users (userName, password, firstName, lastName, streetName, city, PLZ) VALUES (%s, %s, %s, %s, %s, %s, %s)", (username, hash_password, firstname, lastname, streetname, city, plz))
...
def index():
    ...
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO crimeReports_news (date, time, crimeType, location, latLong, title, username, source) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (crimeDate_input.replace('.','/'), crimeTime_input, crimeType_input, crimeLocation_input, crimeLatLong_input, crimeDescription_input, username, "user"))
        con.commit()
        return redirect("/")
        ...
        with open('crimes.json') as json_file:
            ...
            for entry in data:
                ...
                cur.execute("INSERT INTO crimeReports_news (date, time, location, latLong, title, crimeType, source) VALUES (?, ?, ?, ?, ?, ?, ?)", (date.replace('.','/'), time, location, latLong, title, crimeType, source))
                cur.execute("DELETE FROM crimeReports_news WHERE id IN (SELECT t.id FROM crimeReports_news t GROUP BY t.date, t.time, t.title HAVING COUNT(*) > 1);")
                con.commit()

        #update()

        reports_news = cur.execute("SELECT title, location, latLong, source, date, time, crimeType FROM crimeReports_news ORDER BY date DESC, time DESC")
        ...
        tot_crimeType = cur.execute("SELECT crimeType, sum(case when source = 'Polizeipräsident' then 1 else 0 end) as crimeN, sum(case when source = 'user' then 1 else 0 end) as crimeU FROM crimeReports_news GROUP BY crimeType")

        tot_date = cur.execute("SELECT date, sum(case when source = 'Polizeipräsident' then 1 else 0 end) as crimeN, sum(case when source = 'user' then 1 else 0 end) as crimeU FROM crimeReports_news GROUP BY date")

        tot_location = cur.execute("SELECT location, sum(case when source = 'Polizeipräsident' then 1 else 0 end) as crimeN, sum(case when source = 'user' then 1 else 0 end) as crimeU FROM crimeReports_news GROUP BY location")

        all_latLng = cur.execute("SELECT title, date, time, crimeType, location, latLong FROM crimeReports_news")
...
def signalsLog():
    ...
    username = session["user_id"]
    signals_list = cur.execute("SELECT date, time, location, crimeType, title FROM crimeReports_news WHERE username = %s",
                          (username))
