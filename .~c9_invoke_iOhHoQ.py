...
def index():
    ...
    if request.method == "POST":
        ...
        #crimeLocation = {}
        #crimeLocation['lat'] = request.json['lat']
        #crimeLocation['lng'] = request.json['lng']
        ...
        crimeLatLong_input = request.form.get('crimeLocation')
        print(crimeLatLong_input)
        # crimeTitle_input = request.form.get('crimeTitle')
        crimeDescription_input = request.form.get("crimeDescription")

        db.execute("INSERT INTO crimeReports_news (date, time, crimeType, location, latLong, title, username, source) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (crimeDate_input, crimeTime_input, crimeType_input, "berlinweit", crimeLatLong_input, crimeDescription_input, username, "user"))
        return redirect("/")

    else:
        username = session["user_id"]

        reports_news = db.execute("SELECT title, location, latLong, source, date, time, crimeType FROM crimeReports_news")

        reports_users = db.execute("SELECT description as title, location, latLong, date, time, crimeType FROM crimeReports_users")
        ...
        return render_template("index.html", reports_news = reports_news, tot_crimeType = tot_crimeType, tot_date = tot_date, tot_location = tot_location)
...
def signalsLog():
    ...
    username = session["user_id"]
    signals_list = db.execute("SELECT date, time, location, crimeType FROM crimeReports_news WHERE username = %s",
                          (username))
