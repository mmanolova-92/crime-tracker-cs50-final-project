...
def index():
    ...
    news_crime_reports_summary_crimeType = db.execute("SELECT a.crimeType, a.crimeN, b.crimeU FROM (SELECT crimeType, count(id) as crimeN FROM crimeReports_news GROUP BY crimeType) as a LEFT JOIN (SELECT crimeType, count(id) as crimeU FROM crimeReports_users GROUP BY crimeType) as b ON a.crimeType = b.crimeType")
    news_crime_reports_summary_crimeType_df = pd.DataFrame(news_crime_reports_summary_crimeType)
    
    print(news_crime_reports_summary_crimeType_df)

    news_crime_reports_summary_date_crimeType = db.execute("SELECT a.date, a.crimeType, a.crimeN, b.crimeU FROM (SELECT date, crimeType, count(id) as crimeN FROM crimeReports_news GROUP BY date, crimeType) as a LEFT JOIN (SELECT date, crimeType, count(id) as crimeU FROM crimeReports_users GROUP BY date, crimeType) as b ON a.crimeType = b.crimeType and a.date = b.date")
    news_crime_reports_summary_date_crimeType_df = pd.DataFrame(news_crime_reports_summary_date_crimeType)
    print(news_crime_reports_summary_date_crimeType_df)

    news_crime_reports_summary_location = db.execute("SELECT a.location, a.crimeN, b.crimeU FROM (SELECT location, count(id) as crimeN FROM crimeReports_news GROUP BY location) as a LEFT JOIN (SELECT location, count(id) as crimeU FROM crimeReports_users GROUP BY location) as b ON a.location = b.location")
    news_crime_reports_summary_location_d
    print(news_crime_reports_summary_location)

    # news_crime_reports_summary_crimeType
    # user_crime_reports_summary_crimeType



    #for

    return render_template("index.html", reports_news = reports_news)
    ...
    # db.execute("INSERT INTO crimeReports_users (date, time, location, latLong, title, crimeType, description, username) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    #            (crimeDate_input, crimeTime_input, crimeLocation_input, crimeLatLong_input, crimeTitle_input, crimeType_input, crimeDescription_input, username))
