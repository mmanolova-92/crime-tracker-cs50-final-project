...
def index():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO crimeReports_users (date, time, crimeType, location, latLong, title, username, source) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (crimeDate_input, crimeTime_input, crimeType_input, "berlinweit", "52.520008, 13.404954", crimeDescription_input, username, "user"))
        return redirect("/")
...
def signalsLog():
    ...
    username = session["user_id"]
    signals_list = db.execute("SELECT date, time, location, crimeType FROM crimeReports_users WHERE username = %s",
                          (username))
